...
1. **`BodySizeLimitMiddleware`** — Rejects requests whose `Content-Length` header exceeds the configured maximum (default 64 KB; set via `MAX_BODY_BYTES` env var) with a `413` response before FastAPI parses the body.
